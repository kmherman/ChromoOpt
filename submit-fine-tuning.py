...
# set paths here
HOME             = "/gscratch/ilahie/kmherman" #str(Path.home())
...
params = {
    "atom_types"          : ["C", "N", "O"],  # <-- should match pre-trained model param
    "formal_charge"       : [-1, 0, +1],                 # <-- should match pre-trained model param
    "max_n_nodes"         : 150,                          # <-- should match pre-trained model param
    "job_type"            : JOB_TYPE,
    ...
    "sample_every"        : 10,
    "init_lr"             : 1e-6,#1e-4,
    "epochs"              : 500,                         # <-- number of fine-tuning steps
    "batch_size"          : 4,#4,#64,
    "block_size"          : 10,#1000,
    "n_workers"           : 2,#0,
    "sigma"               : 10,                          # <-- see loss function
    "alpha"               : 0.5,                         # <-- see loss function
    "pretrained_model_dir": f"output_{DATASET}/train/job_1/",
    "generation_epoch"    : 22,                          # <-- which pre-trained model epoch to use
    "n_samples"           : 10,                         # <-- how many graphs to sample every step
    "score_components"    : ["s1"],#["target_size=30"],#["s1"],
    "score_thresholds"    : [0.5],#[0.5],#[0.0],#[10],
